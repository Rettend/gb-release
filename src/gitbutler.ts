...
export async function inspectGitButlerWorkspace(
  ...
  if (
    ...
    (parsed as Partial<ButlerStatus>).stacks?.some(
      stack =>
        !stack ||
        typeof stack !== 'object' ||
...
export async function teardownGitButler(
  ...
  expectSuccess(
    await runner.run('but', ['teardown', '--checkout-to', branch], {
      ...options(root, signal),
      stdio: 'inherit',
    }),
...
export async function restoreGitButler(
  ...
  expectSuccess(
    await runner.run('but', ['setup'], options(root, signal)),
    'Release finished, but `but setup` could not restore GitButler workspace mode.',
  )
  expectSuccess(
    await runner.run('but', ['pull'], options(root, signal)),
