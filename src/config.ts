...
function assertRepositoryPath(root: string, path: string, field: string): string {
  ...
  if (
    ...
    throw new ReleaseError(
      `Configuration field "${field}" contains a path outside the repository: ${path}`,
    )
  }
...
export async function loadConfig(
  ...
  const versionFiles = normalizePaths(
    config.versionFiles,
    'versionFiles',
    DEFAULT_CONFIG.versionFiles,
  )
