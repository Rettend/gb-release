...
export async function runRelease(options: RunReleaseOptions): Promise<string> {
  ...
  try {
    ...
    releaseFailed = true;
    const expectedHead = phase === "committed" ? "" : phase === "pushed" ? releaseCommit : releaseBase;
    if (expectedHead && !await repositoryStateMatches(
      runner,
      root,
      config.target.branch,
      expectedHead,
      ...
      ambiguousState = true;
      logger.error("A project command changed the checked-out branch or Git HEAD. GitButler will not be restored automatically.");
    }
    if (phase === "versioned") {
      logger.error("Release files were changed but not committed. Inspect them, then revert or retry.");
    } else if (phase === "committed") {
      logger.error("The local release commit and any tag were kept. Push them explicitly after resolving the error.");
    }
    throw error;
  } finally {
    if (startedInGitButler && shouldRestore && !options.noRestore) {
      ...
      if (statusError) {
        ...
        logger.error(`Could not verify the worktree before GitButler restoration: ${statusError instanceof Error ? statusError.message : String(statusError)}`);
        ...
        try {
          ...
          logger.error(`GitButler restoration also failed: ${restoreError instanceof Error ? restoreError.message : String(restoreError)}`);
        }
      } else if (!clean) {
        logger.warn("GitButler was not restored because the worktree contains release changes.");
      } else {
        logger.warn("GitButler was not restored because the local release state needs manual recovery.");
      }
    }
  }
}
