...
describe('release workflow', () => {
  test('updates, prepares, commits, tags, pushes, and publishes on main', async () => {
    ...
    expect(JSON.parse(await readFile(join(repository.root, 'jsr.json'), 'utf8')).version).toBe(
      '1.0.1',
    )
    ...
    expect(
      await git(repository.runner, repository.remote, ['show', 'main:package.json']),
  ...
  test('reports a partial release and retry command when publishing fails', async () => {
    ...
    expect(await git(repository.runner, repository.remote, ['tag', '--list', 'v1.0.1'])).toBe(
      'v1.0.1',
    )
    expect(
      await git(repository.runner, repository.remote, ['show', 'main:package.json']),
  ...
  test('rejects preparation commands that change HEAD', async () => {
    const repository = await createTestRepository({
      prepare: 'git commit --allow-empty -m unexpected',
    })
    repositories.push(repository)
