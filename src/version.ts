...
export async function readCurrentVersion(root: string, paths: string[]): Promise<string> {
  ...
  if (!currentVersion) throw new ReleaseError('At least one version file is required.')
...
export async function updateVersionFiles(
  ...
  if (!valid(nextVersion)) throw new ReleaseError(`Invalid semantic version: ${nextVersion}`)
