...
async function resolveWorkspace(root: string, workspace: string): Promise<string> {
  ...
  for (const pattern of patterns) {
    ...
    directories.push(...entries.filter((entry) => entry.isDirectory()).map((entry) => resolve(parent, entry.name)));
  }

  for (const directory of directories) {
    const manifest = await readManifest(directory);
    const relativeDirectory = relative(root, directory).replaceAll("\\", "/");
    if (manifest?.name === workspace || relativeDirectory === workspace || basename(directory) === workspace) {
      return directory;
    }
  }
...
async function npmPublishers(
    ...
    for (const segment of command.split("&&")) {
      ...
      if (tokens[0] === "cd") {
        if (!tokens[1] || tokens.length !== 2) {
          throw new ReleaseError("Use `cd <directory> && npm publish` or configure the publisher with --cwd.");
        }
      ...
      if (hasOption(tokens, "--cwd") && !cwdOption) throw new ReleaseError("Publish option --cwd requires a path.");
      if (hasOption(tokens, "--prefix") && !prefixOption) throw new ReleaseError("Publish option --prefix requires a path.");
      ...
      if (tokens.includes("--workspaces") || tokens.includes("-ws")) {
        throw new ReleaseError("Publishing multiple npm workspaces in one command cannot be authenticated safely.");
      }
      ...
      if (!manifest) {
        throw new ReleaseError(`Could not read the package manifest used by publisher: ${relative(root, cwd) || "."}`);
      }
      let registry = optionValue(tokens, "--registry") ?? manifest?.publishConfig?.registry;
      if (hasOption(tokens, "--registry") && !registry) throw new ReleaseError("Publish option --registry requires a URL.");
      if (registry) {
        ...
        if (scope) {
          ...
          if (configuredRegistry.exitCode === 0 && value && value !== "undefined" && value !== "null") {
  ...
  return publishers.filter((publisher, index) => publishers.findIndex((candidate) => (
    candidate.cwd === publisher.cwd && candidate.registry === publisher.registry
  )) === index);
}

function usesJsr(command: string): boolean {
  return /(?:^|\s)(?:bunx|npx|yarn\s+dlx|pnpm\s+dlx)?\s*jsr\s+publish(?:\s|$)/u.test(command)
...
export async function checkRegistryAuthentication(
  ...
  for (const publisher of await npmPublishers(publishCommands, root, runner, signal)) {
    ...
    if (whoami.exitCode !== 0) {
      ...
      expectSuccess(await runner.run(npmCommand, ["login", ...registryArgs], {
        cwd: publisher.cwd,
        stdio: "inherit",
        ...(signal ? { signal } : {}),
      }), "npm login failed.");
    ...
    logger.info(`npm authentication${publisher.registry ? ` (${publisher.registry})` : ""}: ${whoami.stdout.trim()}`);
  }
