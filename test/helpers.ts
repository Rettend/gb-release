...
export async function createTestRepository(
  ...
  await Promise.all(Object.entries(initialFiles).map(async ([path, contents]) => {
    const absolutePath = join(root, path);
    const parent = absolutePath.slice(0, Math.max(absolutePath.lastIndexOf("/"), absolutePath.lastIndexOf("\\")));
...
export class ButlerTestRunner implements CommandRunner {
  ...
  async run(command: string, args: string[], options: CommandOptions): Promise<CommandResult> {
    ...
    if (args[0] === "pull") {
      return { command: "but pull", exitCode: 0, stdout: "", stderr: "" };
    }
    return { command: `but ${args.join(" ")}`, exitCode: 1, stdout: "", stderr: "Unexpected but command" };
  }
