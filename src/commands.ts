...
async function spawnCommand(
  ...
  return await new Promise((resolve, reject) => {
    ...
    const childCommand = usesWindowsCommandShell ? (process.env.ComSpec ?? 'cmd.exe') : command
    ...
    child.once('close', (code, signal) => {
      ...
      resolve({
        ...
        exitCode:
          abortReason instanceof ReleaseError
...
export function expectSuccess(result: CommandResult, message?: string): CommandResult {
  if (result.exitCode !== 0) throw new CommandError(result, message)
