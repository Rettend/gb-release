...
export async function currentBranch(
  ...
  const result = await runner.run("git", ["symbolic-ref", "--quiet", "--short", "HEAD"], options(root, signal));
...
export async function assertCleanWorktree(
  ...
  if (result.stdout.trim()) {
    throw new ReleaseError("The worktree is not clean. Commit or remove all changes before releasing.");
  }
}
...
export async function pullTarget(
  ...
  expectSuccess(result, `Could not fast-forward ${branch} from ${remote}/${branch}. Resolve it manually.`);
}
...
export async function assertTagAvailable(
  ...
  if (local.exitCode === 0) {
    throw new ReleaseError(`Tag already exists locally: ${tag}`);
  }
  if (local.exitCode !== 1) {
  ...
  if (remoteTag.exitCode === 0) {
    throw new ReleaseError(`Tag already exists on ${remote}: ${tag}`);
  }
...
export async function commitRelease(
  ...
  if (preStaged.stdout) {
    throw new ReleaseError("A preparation command staged files. Preparation commands must not modify the Git index.");
  }
...
export async function validateReleaseCommit(
  ...
  if (parent.stdout.trim() !== expectedParent) {
    throw new ReleaseError("The release commit was not created directly on the validated target commit.");
  }
...
export async function assertReleaseReadyToPush(
  ...
  if (checkedOutBranch !== branch) {
    throw new ReleaseError(`A project command changed the checked-out branch to "${checkedOutBranch}".`);
  }
  const head = await readHead(runner, root, signal);
...
export async function assertPublishOnlyRelease(
  ...
  if (validTag.exitCode !== 0) throw new ReleaseError(`Configured tag name is not a valid Git ref: ${tag}`);

  const checkedOutBranch = await currentBranch(runner, root, signal);
  if (checkedOutBranch !== branch) {
    throw new ReleaseError(`Publication retry must run from "${branch}", not "${checkedOutBranch}".`);
  }
  const head = await readHead(runner, root, signal);
  ...
  if (localTag.exitCode !== 0) {
    throw new ReleaseError(`Cannot retry publication because local release tag ${tag} does not exist.`);
  }
  ...
  if (!peeledTag || !remoteHead || head !== localRelease || head !== peeledTag || head !== remoteHead) {
    throw new ReleaseError(
